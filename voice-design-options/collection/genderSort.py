...
def main():
    ...
    print("  [m] Male voice | [f] Female voice | [s / Space] Skip | [q] Quit\n")
    ...
    try:
        for idx, wav_file in enumerate(wav_files, start=1):
            ...
            if key == 'q':
                ...
                try:
                    dest = female_dir / wav_file.name
                    shutil.move(str(wav_file), str(dest))
                    print("[Moved to 'female/']")
                except Exception as e:
                    print(f"[Error moving file: {e}]")
            elif key in ['s', ' ', '\r', '\n']:
                print("[Skipped]")
