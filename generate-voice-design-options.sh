...
#
# Generate several candidate character voices, with three
# stochastic variations of each voice description.
#
# Designed for the existing project at:
#   /mnt/storage/projects/qwenVoice
#
# Everything remains underneath this directory.
# ============================================================
...
OUTPUT_DIR="${ROOT}/voice-design-options"
CACHE_DIR="${ROOT}/cache/huggingface"
TMP_DIR="${ROOT}/tmp"
GENERATOR="${ROOT}/generate_voice_design_options.py"
VOICE_FILE="${1:-${ROOT}/voice-designs.json}"


# ------------------------------------------------------------
# Keep everything on /mnt/storage
# ------------------------------------------------------------
...
export QWEN_THREADS=6


echo
echo "============================================================"
echo " Qwen3-TTS 1.7B VoiceDesign BF16"
echo " Character Voice Auditions"
echo "============================================================"
echo
echo "Project:"
echo "  ${ROOT}"
...
# ------------------------------------------------------------
# Make sure existing Qwen environment is available
# ------------------------------------------------------------

if [[ ! -x "${PYTHON}" ]]; then
    echo "ERROR: Existing Qwen virtual environment was not found:"
    echo "  ${PYTHON}"
    echo
    exit 1
fi
...
if ! "${PYTHON}" -c 'import qwen_tts' >/dev/null 2>&1; then
    echo "ERROR: qwen_tts is not importable from the existing venv."
    exit 1
fi


# ------------------------------------------------------------
# Download VoiceDesign checkpoint if necessary
# ------------------------------------------------------------
...
PY
    ...
    echo "VoiceDesign model already present."
    echo "  ${MODEL_DIR}"
...
# ------------------------------------------------------------
# Write actual generation program
# ------------------------------------------------------------
...
echo
echo "Starting VoiceDesign candidate generation..."
echo

if [[ ! -f "${VOICE_FILE}" ]]; then
    echo "ERROR: Voice description JSON not found:"
    echo "  ${VOICE_FILE}"
    exit 1
fi

echo "Voice descriptions:"
echo "  ${VOICE_FILE}"
echo

"${PYTHON}" "${GENERATOR}" --voices "${VOICE_FILE}"
